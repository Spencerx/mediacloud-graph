...
function showLabels() {
    if (!d3.select('#show-labels').filter(':checked').empty()) {
        d3.selectAll('text')
...
function redraw() {
    ...
    d3.selectAll('text').style('font-size', function(n) {             
...
function go(frame, i) {
    ...
    group
        ...
        .classed('hidden', function(n) { return !SHOW_LABELS || fontSize(radius(n.size)) < LABEL_SIZE_CUTOFF; })
